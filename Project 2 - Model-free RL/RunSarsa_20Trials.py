...
for epsilon in epsilon_rng:
    ...
    for i in tqdm(range(20)):
        ...
        if i==0: 
            Q = pd.DataFrame.from_dict(sarsa.Q, orient='index')
        else: 
            Q = pd.concat([Q,pd.DataFrame.from_dict(sarsa.Q, orient='index')])
...
def initialize_plot():
    ...
    plt.ylabel('G_0 Mean')
    plt.xlabel('Episode')
    plt.title('SARSA with Changing Epsilon [Average Mean over 20 Trials]')
...
def initialize_plot():
    ...
    plt.xlabel('Episode')
    plt.title('SARSA with Changing Epsilon [Average SD over 20 Trials]')
    plt.rcParams['figure.dpi'] = 150
    return plt
